...
public class Sale {
    ...
    public SaleInfoDTO addBoughtItem(ItemDTO boughtItem) {
        ...
        try {
            ...
            totalPrice = totalPrice.subtract(boughtItem.price());
            totalVat = totalVat.subtract(boughtItem.price().multiply(boughtItem.vat()));
            throw Error;
        }
    ...
    private ItemDTO calculateRunningTotal(ItemDTO boughtItem) {
        if (boughtItem == null) {
            throw new IllegalArgumentException("Cannot calculate total for null item");
        }
        ...
        // Since item from inventory is base price, but we need to show the full price
        // to the view.
     ...
     *
     * @return The bought items. Returns an empty list if no items have been
     * added.
     */
     ...
     * @throws IllegalArgumentException If the amount paid is less than the
     * total price.
     */
     ...
     * @throws IllegalArgumentException If discountInfo is null or if discount
     * amount is negative or if discount amount is greater then total price.
     */
